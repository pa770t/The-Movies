<script setup>
    ...
    const movies = computed(() => store.getters.getNowShowingMovies)
    const isAuthenticated = computed(() => store.state.isAuthenticated)

    onMounted(() => {
        if (isAuthenticated.value) {
            store.dispatch('getNowShowingMoviesFromStore')
        }

    })

    const previousPage = (url) => {
        store.dispatch('getNowShowingMoviesFromStore', url)
    }
...
<template>
    <section class="mt-2 container">
        <p class="text-3xl border-b border-dashed pb-2 mt-10">Now Showing in Theaters</p>
